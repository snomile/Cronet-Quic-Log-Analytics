...
def process_host_resolver_impl_job(resolver_jobs):
    ...
    for key, values in resolver_jobs.items():
        ...
        for event in values:
            ...
            if "HOST_RESOLVER_IMPL_JOB_STARTED" == event.event_type and original_data.get('dns_begin_time') is None:
                original_data['dns_begin_time'] = event.time_int
            if "HOST_RESOLVER_IMPL_ATTEMPT_FINISHED" == event.event_type and original_data.get('dns_end_time') is None:
